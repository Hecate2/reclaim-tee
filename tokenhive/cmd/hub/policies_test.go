...
// route surface admits nobody rather than scheduling refusals.
func TestAdmitAgainstPolicyLooksUpByHostAndPath(t *testing.T) {
	...
	if err := admitAgainstPolicy(&doc, "api.openai.com")(reg); err != nil {
		t.Fatalf("a whitelisted upstream was refused: %v", err)
	}

	// The same agent, judged on a host the deployment does not reach.
	if err := admitAgainstPolicy(&doc, "evil.example.com")(reg); err == nil {
	...
	err := admitAgainstPolicy(&narrow, "api.openai.com")(reg)
	if err == nil {
	...
	if err := admitAgainstPolicy(nil, "api.openai.com")(reg); err == nil {
