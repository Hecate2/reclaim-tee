// Package tencent is the TokenHive platform adapter skeleton for Tencent Cloud
// confidential computing (TEE encrypted-computing instances and confidential
// VMs backed by Intel SGX / TDX / AMD SEV).
//
...
// Tencent's remote-attestation protocol must be validated and wired into the
// Verifier before this platform may admit trusted work. Until then:
//
//   - NewTencentCloud refuses to start on a real host unless Config.AllowUntrusted
//     is set, and AllowUntrusted is explicitly a wiring-only mode that signs
//     with an UNATTESTED software key;
//   - Verifier.CheckEvidence refuses every tencent receipt with
...
// Config controls the identity emitted by the Tencent Cloud adapter.
type Config struct {
	...
	// It exists ONLY to exercise the wiring end to end on a Tencent instance
	// before the attestation path is implemented. Receipts it signs carry the
	// "tencent" platform string but are refused by Verifier by design. Never
...
// NewTencentCloud detects the host and returns an adapter for a Tencent Cloud
...
func NewTencentCloud(config Config) (*Adapter, error) {
...
func newAdapter(config Config, detect func() (string, bool)) (*Adapter, error) {
	...
	tech, ok := detect()
	if !ok {
		return nil, errors.New("tencent adapter requires a Tencent Cloud confidential-computing guest (SGX / TDX / SEV-SNP device not found)")
	}
	if !config.AllowUntrusted {
		return nil, fmt.Errorf("tencent attestation verification is not implemented; refusing to start (%s detected). Rebuild with -tags cloud and set AllowUntrusted ONLY for wiring tests: %w", tech, platform.ErrAttestationNotImplemented)
	}
	epoch, err := dev.NewEpoch(platform.PlatformTencentCloud, "tokenhive-tee")
	if err != nil {
		return nil, fmt.Errorf("mint tencent dev epoch: %w", err)
	}
