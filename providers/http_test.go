...
func TestShouldHandleOPRFReplacementsInChunkedResponse(t *testing.T) {
	...
	// When Hash is set to "oprf", the named capture group content should be
	// marked as a redaction with Hash field set (for OPRF replacement)
	hasOPRFRedaction := false
	oprfContent := ""
	for _, red := range redactions {
		if red.Hash == "oprf" {
			hasOPRFRedaction = true
			// Extract the content that would be OPRF-replaced
			oprfContent = string(response[red.Start : red.Start+red.Length])
			break
		}
	}

	if !hasOPRFRedaction {
		t.Error("Expected at least one OPRF redaction (with Hash='oprf') in chunked response")
	}

	// The OPRF-marked content should be "John" (the captured group value)
	if oprfContent != "John" {
		t.Errorf("Expected OPRF redaction to contain 'John', got: %q", oprfContent)
	}

	// Verify the revealed content has the JSON structure but not the captured value
	revealed := ""
	start := 0
	for _, red := range redactions {
		if red.Start > start {
	...
	// The revealed content should have the name field structure but not the value
	if !strings.Contains(revealed, `"name":"`) || !strings.Contains(revealed, `"`) {
		t.Errorf("Expected revealed content to contain JSON structure, got: %q", revealed)
	}
	if strings.Contains(revealed, "John") {
		t.Errorf("Expected 'John' to be redacted (marked for OPRF), but it appears in revealed content")
	}

	t.Logf("OPRF chunked response test passed with %d redactions, OPRF content: %q", len(redactions), oprfContent)
}
...
<body>
	...
	if !hasOPRFRedaction {
		t.Error("Expected at least one OPRF redaction (with Hash='oprf')")
	}

	// The OPRF-marked content should be "Example Domain"
	if oprfContent != "Example Domain" {
		t.Errorf("Expected OPRF redaction to contain 'Example Domain', got: %q", oprfContent)
	}

	// Verify the revealed content has the title tags but not the content
	revealed := ""
	start := 0
	for _, red := range redactions {
		if red.Start > start {
	...
	// The revealed content should have the title tags but not the domain name
	if !strings.Contains(revealed, "<title>") || !strings.Contains(revealed, "</title>") {
		t.Errorf("Expected revealed content to contain title tags, got: %q", revealed)
	}
	if strings.Contains(revealed, "Example Domain") {
		t.Errorf("Expected 'Example Domain' to be redacted (marked for OPRF), but it appears in revealed content")
	}

	t.Logf("OPRF redaction test passed with %d redactions, OPRF content: %q", len(redactions), oprfContent)
}
...
func TestGetResponseRedactions_NonSuccess_ShouldThrow(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "expected status 2xx, got 404 (Not Found)") {
...
func expectRedactionAt(t *testing.T, redactions []shared.ResponseRedactionRange, index int, expectedStart int, expectedLength int) {
...
var chunkedResp = func() []byte {
	b64 := `SFRUUC8xLjEgMjAwIE9LDQpEYXRlOiBNb24sIDA0IFNlcCAyMDIzIDE1OjQ0OjMzIEdNVA0KQ29u
...
func TestGetResponseRedactions_BookfaceChunked(t *testing.T) {
	...
	if len(reds) != len(expected) {
		...
		for i, r := range reds {
			t.Logf("  Redaction %d: [%d:%d] (length %d)", i, r.Start, r.Start+r.Length, r.Length)
		}
	...
	t.Logf("Bookface chunked test passed with %d redactions", len(reds))
}
